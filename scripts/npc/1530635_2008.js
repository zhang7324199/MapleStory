    ...
    if (status == 0) {
        ...
        switch (selection) {
            case 0://金币
                ...
                break;
            case 1://点券
                cm.sendGetNumber("请输入您想要输入的点券数量", 0, 0, 10000);
                break;
            case 2://抵用
                cm.sendGetNumber("请输入您想要输入的抵用数量", 0, 0, 10000000);
                break;
            case 3://现金
                cm.sendGetNumber("请输入您想要输入的现金数量", 0, 0, 100);
                break;
            case 4://道具
        ...
        cm.sendOk("生成红包数据成功！！");
        cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "哇塞， " + cm.getChar().getName() + " 开启了限量抢红包活动。赶紧抓紧时间抢吧。");
        cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "哇塞， " + cm.getChar().getName() + " 开启了限量抢红包活动。赶紧抓紧时间抢吧。");
        cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "哇塞， " + cm.getChar().getName() + " 开启了限量抢红包活动。赶紧抓紧时间抢吧。");
        cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "哇塞， " + cm.getChar().getName() + " 开启了限量抢红包活动。赶紧抓紧时间抢吧。");
        cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "哇塞， " + cm.getChar().getName() + " 开启了限量抢红包活动。赶紧抓紧时间抢吧。");
...
function getLuckeyMoney(which) {
	...
	var ps = conn.prepareStatement("SELECT * FROM LuckMoneyData where id =" + monArr[which] + " and  used < AllowPeople")
    var getMoney = ps.executeQuery();
    //TODO 再次判断是否已经领取
    ...
    while (getMoney.next()) {
        switch (parseInt(getMoney.getString("luckmoneytype"))) {
            case 0://金币
                ...
                cm.sendOk("领取成功啦！！！");
                cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 金币红包。");
                break;
            case 1://点券
                cm.gainNX(1, parseInt(getMoney.getString("LuckyMoney")));
                cm.sendOk("领取成功啦！！！");
                cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 点卷红包。");
                break;
            case 2://抵用
                cm.gainNX(2, parseInt(getMoney.getString("LuckyMoney")));
                cm.sendOk("领取成功啦！！！");
                cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 抵用卷红包。");
                break;
            case 3://现金
                cm.addHyPay(-parseInt(getMoney.getString("LuckyMoney")));
                cm.sendOk("领取成功啦！！！");
                cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 现金红包。");
                break;
            case 4://道具
                cm.gainItem(parseInt(getMoney.getString("LuckyMoney")), 1);
                cm.sendOk("领取成功啦！！！");
                cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + ii.getName(parseInt(getMoney.getString("LuckyMoney"))) + " 道具红包。");
                break;
        }
...
function iUsed(uid, times) {
	var conn = cm.getConnection();
    var usedpp = conn.prepareStatement("update LuckMoneyData set used = ? where id =" + uid);
...
function findLuckeyMoney(index) {
    ...
    if (index == 0) {//0显示可用， 1显示所有
        getMoney = conn.prepareStatement("SELECT * FROM LuckMoneyData where used < AllowPeople").executeQuery();
    } else if (index == 1) {
        getMoney = conn.prepareStatement("SELECT * FROM LuckMoneyData").executeQuery();
    }
    var text = "";
    var i = 0;
    while (getMoney.next()) {
        text += "#L" + i + "# 第" + (i + 1) + "个神秘哒红包!\r\n";
...
function setLuckeyMoney(monType, monNumber, monMax) {
	var conn = cm.getConnection();
    var inserLuckeyMoney = conn.prepareStatement("INSERT INTO LuckMoneyData(id, luckmoneytype, LuckyMoney, AllowPeople, used) value(?,?,?,?,?)");
...
function delMoney(uid) {
	var conn = cm.getConnection();
    var delectData = conn.prepareStatement("delete from LuckMoneyData where id = " + uid + "");

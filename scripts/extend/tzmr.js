        ...
        if (cm.getPlayer().getClient().getChannel() == 1 || cm.getPlayer().getClient().getChannel() == 2 || cm.getPlayer().getClient().getChannel() == 3) {
            if (status == 0) {
                if (cm.getMap().getId() == 802000821) {
                    ...
                    cm.sendSimple("- #e#d英雄救美#k#n:\r\n\r\n#b阿利博士实行生化魔人改造试验失败了，因此生化魔人绑架了#r#p9310114##b，但是由于生化魔人的威力太过强大，需要年轻的冒险家们进行解救。杀死怪物后会出现NPC.请靠近NPC按住采集键。开始疯狂的领取奖励吧!不要松开!!!魔方点券抵用本。#k\r\n副本要求：\r\n#r1). 可在1,2,3线可挑战。\r\n2). 组队员等级必须要在" + minLevel + "级以上。\r\n3). 组队员必须要" + minPartySize + "人以上，" + maxPartySize + "人以下。\r\n\r\n#L0#[执行]生化魔人解救战#l")
                }
            } else if (status == 1) {
                if (selection == 0) {
                    if (cm.getParty() == null) { // 没有组队
                        ...
                        if (cm.getPQLog("欧拉拉") < 2) {
                            ...
                            if (cm.getPlayerCount(802000821) == 0) {
                                        em.startInstance(cm.getParty(), cm.getMap(), 198);
                                        cm.setPQLog("欧拉拉");
                                        cm.spouseMessage(0x20, "[生化魔人欧啦啦] ：玩家 " + cm.getChar().getName() + " 进入生化魔人 欧啦啦 栖息之地 <无引力实验室>，请小心。");
                                        cm.dispose();
                                        return;
                                    } else {

...
else if (cm.getPQLog("InvasionEvent") >= 1){
cm.sendOk("您已经进入过。"); 
cm.dispose();
}
else if (cm.getEventCount("InvasionEvent") >= 1){
cm.sendOk("您已经进入过。"); 
cm.dispose();
}else{
var em = cm.getEventManager("InvasionEvent");
if (em == null) {
cm.sendOk("出错啦,请联系GM.");
cm.dispose();
}else{
...
cm.spouseMessage(0x20,"『次元挑战』" + " : " + "恭喜" + cm.getChar().getName() + ",和他的队友开始了次元挑战，祝他取得好的成绩");

/* 
 *  日常任务
 *  任务初始ID 200106，子任务依次加1，新任务分类的初始ID需间隔100，如100200
 */
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == -1) {
            ...
            accepttimes = maxtimes - cm.getPQLog("寻宝任务");
            text = "\t\t\t#e每日寻宝 - 金利奇的黄金袋子#n\r\n\r\n";
            text += "我的财富，遍布世界各地……\r\n";
            text += "今日剩余寻宝次数：#r" + accepttimes + "#k 次\r\n";
            text += "#r#L999#寻宝任务简介#l#k\r\n\r\n";
            if (cm.getPlayer().getTreasureQid() > 0) {  // 判断是否接取了任务
                if (cm.isQuestFinished(cm.getPlayer().getTreasureQid())) { // 判断是否完成任务
            ...
            if (selection == 999) {
                ...
                cm.sendYesNo("是否要花费#r800#k点购买一个黄金罗盘？");
                ...
                // 重新接受任务
                cm.getPlayer().setTreasureQid(questid);
...
function spawnNpc(mapid) {
    var point;
    do {
        point = new java.awt.Point((Math.random() * 588) - 270, (Math.random() * 588) - 270);

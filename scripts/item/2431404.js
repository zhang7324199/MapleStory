    ...
    if (mode == -1) {
        ...
        if (a == -1) {
            ...
            for (var i = 0; i < PayLevel.length; i++) {
                if (RMB >= PayLevel[i][1]) {
                    TEMP = im.getBossLogAcc("累计" + (parseInt(PayLevel[i][1])) + "元礼包") == -1 ? "#g(已领取)#b" : "#r(未领取)#b";
            ...
            if (pass) {
                ...
                if (im.getBossLogAcc("累计" + ChoosenRMB + "元礼包") == -1) {
            ...
            if (RMB < ChoosenRMB) {
                ...
                im.dispose();
            } else if (im.getBossLogAcc("累计" + ChoosenRMB + "元礼包") == -1) {
                im.sendOk("你不能重复领取礼包哟！\r\n 一个帐号只能领取一次！");
                im.dispose();
            } else {
                for (var i = 0; i < WeaponList.length; i++) {
                ...
                if (CheckSpace == false) {
                    ...
                    im.setBossLogAcc("累计" + ChoosenRMB + "元礼包", -2);
                    im.playerMessage(1, "领取" + ChoosenRMB + "元礼包成功！");
                    im.channelMessage(0x18, "『累计充值奖励』" + " : " + "玩家 " + im.getChar().getName() + " 领取了累计充值" + ChoosenRMB + "元礼包！");
                    im.dispose();
                }
            }
        }//a
